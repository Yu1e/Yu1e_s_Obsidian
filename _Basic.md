<%*
// Соединены шаблоны основной (_basic) и 30-СЕКУНДНОЕ ПЕРЕИМЕНОВАНИЕ после создания заметки, для заметок, которые открываешь и сразу начинаешь в них печатать. Использует шаблоны "10_words..." и "yaml". Чат: https://www.perplexity.ai/search/chem-otlichaiutsia-shablony-30-VKdn.1w_Smiflpaaj5xB_w
...
// Вместо минимальной шапки берём тот же генератор YAML, что и в базовом шаблоне.
tR = await tp.user.yaml.generate(tp);
%>