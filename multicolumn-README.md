...
| **Flexible output** | Supports 2–5 columns with optional headers |
...
4. Choose the number of columns (2–5)
...
weight = character_count + (line_count - 1) × LINE_WEIGHT
Higher values → line breaks count more → better for narrow columns
Lower values → character count dominates → better for wide columns
...
[Select this text and run template ↓]
...
2. Run the template
3. Choose "3 columns"
4. Done — content is automatically redistributed
